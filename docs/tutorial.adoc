...
Let's generate one.  We are going to use the genvivprj_func function.  Documentation for functions in vivgen_functions.cmake is in vivgen_functions.adoc .
...
file(GLOB hdllistfile hdl/filelist.txt)
file(GLOB tblistfile tb/filelist.txt)
file(GLOB datalistfile testdata/filelist.txt)

readFilelist(hdlfiles ${hdllistfile})
readFilelist(tbfiles ${tblistfile})
readFilelist(datafiles ${datalistfile})
----
...
To generate a bit file, we use the vivnonprjbitgen_func function in vivgen_functions.cmake (documented in vivgen_functions.adoc).
...
file(GLOB hdllistfile hdl/filelist.txt)
file(GLOB tblistfile tb/filelist.txt)
file(GLOB lateconstrlistfile constraints/unscopedlate/filelist.txt)

readFilelist(hdlfiles ${hdllistfile})
readFilelist(tbfiles ${tblistfile})
readFilelist(lateconstrfiles ${lateconstrlistfile})
